...
本格式的**约定限制**有：每一种相同牌面值的牌**只有四张**。立直交出的供托**只有一个且为1000点**。使用**没有花牌**的134张麻雀牌。
...
开局最先翻开表dora指示牌以`dora=tile`表示。如`dora=E`。也可以简记为`d=tile`。
...
开杠后的新dora以也由以`dora=tile`或`d=tile`表示。紧跟在开dora的摸打后。如：`d=E`。
...
增量方式以`$End num1 num2 num3 num4`表示，分别为东南西北家的本局与**本局起始点数相差的点数**（**舍弃末尾的两个零**）。

#### 绝对值方式

绝对值方式以`$Finish num1 num2 num3 num4`表示，或简写成`$Fin num1 num2 num3 num4`。分别为东南西北家的本局与**本局结束点数**（**舍弃末尾的两个零**）。